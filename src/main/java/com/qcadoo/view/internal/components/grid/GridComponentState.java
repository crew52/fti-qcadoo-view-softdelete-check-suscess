    ...
    protected class GridEventPerformer {
        ...
        public void softDeleteSelectedEntity(final String[] args) {
            ...
            for (Long selectedId : selectedEntities) {
                ...
                // Lấy deleted theo kiểu Boolean an toàn
                Boolean wasDeleted = (Boolean) entity.getField("deleted");

                // Nếu đã deleted rồi → bỏ qua
                if (Boolean.TRUE.equals(wasDeleted)) {
                    continue;
                }

                // Gán giá trị soft delete
                ...
                // Copy lỗi về UI nếu có
                copyFieldValidationMessages(saved);

                // Nếu có lỗi → trạng thái chung = fail
                if (!saved.isValid()) {
                    allDeletedSuccessfully = false;
                }
            }

            // CHỈ hiển thị thông báo khi tất cả đều delete thành công
            if (allDeletedSuccessfully) {
                if (selectedEntities.size() == 1) {
                    addTranslatedMessage(
                            translateMessage("deleteMessage"),
                            MessageType.SUCCESS
                    );
                } else {
                    addTranslatedMessage(
                            translateMessage("deleteMessages",
                                    String.valueOf(selectedEntities.size())),
                            MessageType.SUCCESS
                    );
                }
            }
